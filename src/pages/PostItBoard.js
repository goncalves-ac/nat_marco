...
const PostItBoard = () => {
  ...
  useEffect(() => {
    fetch("https://nataliaemarcos.online/getMessages.php")
      ...
      .then((data) => {
        // Filtrar mensagens onde o campo "message" não está vazio
  ...
  const openModal = (msg) => {
    setModalContent(msg);
    setModalOpen(true);
    // Adiciona o estilo ao abrir o modal
    const navList = document.querySelector(".nav-list");
    if (navList) {
      navList.style.backgroundImage = "none";
    }
  ...
  const closeModal = () => {
    ...
    // Remove o estilo inline para restaurar o valor original do CSS
    const navList = document.querySelector(".nav-list");
    if (navList) {
      navList.style.removeProperty("background-image");
    }
  };
        ...
        <div className="post-it-board-container">
          <div className="post-it-board">
            {currentMessages.map((msg) => {
              ...
              const backgroundColor = isRead ? "#90ee90" : getRandomColor();
